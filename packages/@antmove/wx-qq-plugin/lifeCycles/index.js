...
module.exports = {
  ...
  onCompiling(fileInfo, ctx) {
    ...
    if (isTypeFile('.wxml', fileInfo.path)) {
      ...
      content = content.replace(/\.wxs/g, '.qs')
      fs.outputFileSync(fileInfo.dist.replace(/\.wxs$/, '.qs'), content)
    } else {
      let content
  ...
  async compiled(ctx, cb = () => {}) {
    ...
    const targetPath = path.join(ctx.output, `${Config.library.customComponentPrefix}/.config.json`)
