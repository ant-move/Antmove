...
module.exports = function(fileInfo, ctx) {
  ...
  fileInfo.dist = fileInfo.dist.replace(/\.wxss/, '.qss')
  let cssContent = fs.readFileSync(fileInfo.path, 'utf8') || ''
  cssContent = prettierCode(cssContent, 'scss')

  cssContent = cssContent.replace(/\.wxss"/g, '.qss";').replace(/\.wxss'/g, '.qss\';')
  ...
  cssContent = cssContent.replace(/@import\s+['|"](\S+)['|"]/g, (...$) => {
    let rule = $[1]
    if ((rule[0] !== '/' && rule[0] !== '.')) {
      const tempPath = path.join(fileInfo.dirname, rule.replace(/\.wxss'*/g, '.qss'))
  ...
  cssContent = prettierCode(cssContent, 'scss')
  if (fileInfo.deep && /\S*\.wxml/.test(fileInfo.filename)) { return }
