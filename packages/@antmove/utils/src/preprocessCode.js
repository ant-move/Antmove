...
module.exports = {
  precessRelativePathOfCode(code, filepath, dir, isComponent) {
    if (isComponent) {
      return code
    }
    return code.replace(/\.+((\/|\\)\.+)+(\/|\\)*\w+/g, (...r) => {
  ...
  precessAbsolutePathOfCode(code, filepath, dir) {
    return code.replace(/["'](((\/|\\)(\w|[-_])+)+(\/|\\)*\w+)/g, (...r) => {
      const prefix = r[0][0]
      const requireFilePath = dir + r[0].substring(1)
