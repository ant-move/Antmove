...
const method = {
  ...
  dateFormat(date, fmt) {
    ...
    for (const k in o) { if (new RegExp(`(${k})`).test(fmt)) { fmt = fmt.replace(RegExp.$1, (RegExp.$1.length === 1) ? (o[k]) : ((`00${o[k]}`).substr((String(o[k])).length))) } }
    return fmt
  },
  reportSpeed(speed) {
    if (!speed.showReport) {
...
module.exports = {
  report(date, reportData = {}) {
    const finishiDate = Number(new Date())
    ...
    if (reportData.type === 'computedTime') {
      const uesTime = finishiDate - date
    ...
    if (reportData.type === 'parse') {
      const time = method.dateFormat(date, 'hh:mm:ss')
      logData = {
        time,
    ...
    if (reportData.type === 'compile') {
      ...
      const _finishiDate = new Date()
      const cycle = `${_finishiDate - date}ms`
      const time = method.dateFormat(_finishiDate, 'hh:mm:ss')
      logData = {
        time,
  ...
  reportTable(table = {}) {
    ...
    for (const key in table.tableInfo) {
      if (table.tableInfo.hasOwnProperty(key)) {
        new Line(outputBuffer)
          .column(key, 16)
  ...
  reportDist: (version, distPath, toolData = {}) => {
