...
module.exports = function(config = {}, target, edition) {
  ...
  const isExternal = edition
  const befor = target.split('-')[0]
  const after = target.split('-')[1]
  function tansformTarget(_target) {
    let str = ''
    switch (_target) {
      case 'wechat': str = '微信'
        break
  ...
  const tansformBefor = tansformTarget(befor)
  const tansformAfter = tansformTarget(after)
  const {
    ComponentInfo,
    ApiInfo,
    LifeInfo,
  ...
  function generateSideBarJson(res) {
    const p = path.join(__dirname, '../../../../../../antmove-website/website/config/heardLinks.js')
    const headA = str1
    const headB = str2
    let json = {}
    let getPath = ''
  ...
  function renderApiDoc(_apiAll, type) {
    const apiDoc = []
    let _str = ''
    const header = ['函数名', '说明', `${tansformBefor}小程序`, `${tansformAfter}小程序`, '是否支持']
    const retValheader = ['差异属性', '说明', '差异类型']
    const paramsHeader = ['差异参数', '说明', '差异类型']
    let apiAll = []
    if (type === 'api') {
    ...
    apiAll.forEach((apiName) => {
      ...
      Object.keys(apiName.body)
        .forEach((fnName) => {
          const arr = []; const retValArr = []; const paramsArr = []; let
            valType = ''
          arr.push(fnName)
          const _apiInfo = apiName.body[fnName]
          if (_apiInfo.desc) {
            arr.push(_apiInfo.desc)
          } else {
            arr.push(' ')
          }
          Object.keys(_apiInfo.url).forEach((v) => {
            if (_apiInfo.url[v]) {
  ...
  function renderComponentDoc(obj, type) {
    const componentDoc = []; let _str = ''; let
      ComponentsInfo = []
    const header = ['差异属性', '说明', '是否支持', '备注']
    const _header = ['属性名', '描述', `${tansformBefor}小程序`, `${tansformAfter}小程序`, '是否支持']
    if (type === 'components') {
      ComponentsInfo = obj.ComponentsInfo
    } else {
      ComponentsInfo = obj.jsonInfo
    }
    ComponentsInfo.forEach((fnName, i) => {
      const componentObj = {}
      _str = ''
      _str += '---\n'
      ...
      Object.keys(ComponentsInfo[i].body)
        .forEach((attrName) => {
          const arr = []; const
            _arr = []
          const attrInfo = ComponentsInfo[i].body[attrName]
          _str += h2(attrName)
          if (type === 'components') {
            _str += `${attrInfo.desc} \n `
          } else {
            ...
            Object.keys(attrInfo.url).forEach((v) => {
              if (attrInfo.url[v]) {
          ...
          if (attrInfo.props) {
            ...
            Object.keys(propsObj)
              .forEach((attr) => {
                arr.push(attr)
                const _attr = attrInfo.props[attr]
                let attrDesc = _attr.desc
                if (_attr.params) {
                  Object.keys(_attr.params).forEach((prm) => {
                ...
                const attrSupport = attrInfo.props[attr].status
  ...
  function renderUnSupportApiDoc(obj) {
    const arr = []; let str = ''; const
      id = `${befor}-${after}-unsupport-apis`
    Object.keys(obj)
      .forEach((key) => {
        const value = obj[key]
        if (value.status === 2) {
  ...
  function renderUnSupportComponentsDoc(obj) {
    const arr = []; let str = ''; const
      id = `${befor}-${after}-unsupport-components`
    Object.keys(obj)
      .forEach((key) => {
        const value = obj[key]
        if (value.status === 2) {
  ...
  function renderUnSupportLifecircleDoc(obj) {
    const arr = []; let str = ''; const
      id = `${befor}-${after}-unsupport-lifeCircle`
    Object.keys(obj)
      .forEach((key) => {
        const value = obj[key]
        if (value.status === 2) {
  ...
  const apiRes = renderApiDoc(ApiInfo, 'api')
  const componentRes = renderComponentDoc(ComponentInfo, 'components')
  const lifeRes = renderApiDoc(LifeInfo, 'life')
  const jsonRes = renderComponentDoc(JsonInfo, 'json')
