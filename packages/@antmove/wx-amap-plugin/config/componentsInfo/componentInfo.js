module.exports = {
  descObject: {
    'movable-view': {
      ...
      desc:
        '在页面中可以拖拽滑动。movable-view必须在 movable-area 组件中，并且必须是直接子节点，否则不能移动。',
      props: {
        direction: {
          type: 7,
          status: 0,
    ...
    'cover-view': {
      ...
      desc:
        '可覆盖的原生组件包括 map、video、canvas、camera、live-player、live-pusher只支持嵌套 cover-view、cover-image，可在 cover-view 中使用 button。组件属性的长度单位默认为px，2.4.0起支持传入单位(rpx/px)。',
      props: {
        'scroll-top': {
          type: 0,
          status: 2,
    ...
    'scroll-view': {
      ...
      props: {
        ...
        'enable-back-to-top': {
          ...
          status: 0,
          desc:
            '当点击 iOS 顶部状态栏或者双击安卓标题栏时，滚动条返回顶部，只支持竖向',
        },
        bindscrolltoupper: {
          type: 1,
          status: 0,
    ...
    audio: {
      ...
      desc:
        '1.6.0版本开始，该组件不再维护。建议使用能力更强的 wx.createInnerAudioContext 接口',
      props: {
        id: {
          type: 7,
          status: 0,
    ...
    map: {
      ...
      props: {
        ...
        controls: {
          name: '控件',
          desc:
            '控件（即将废弃，建议使用 cover-view 代替在地图上显示控件，控件不随着地图移动。即将废弃，请使用 cover-view',
          type: 5,
          status: 0,
          props: {
            id: {
    ...
    'web-view': {
      ...
      desc:
        '会自动铺满整个小程序页面，个人类型的小程序暂不支持使用。客户端 6.7.2 版本开始，navigationStyle: custom 对 web-view 组件无效',
      props: {
        src: {
          type: 7,
          status: 0,
