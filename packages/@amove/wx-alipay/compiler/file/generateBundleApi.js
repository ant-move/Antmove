...
const wrapApis = require('../../runtime/__api/my.js')
const wrapApisInfo = require('../config/apiInfo/index').descObject

const entry = path.join(__dirname, '../../runtime/__api')

/**
 * 
 * @param {*} output 
 */
function generate(output, Config) {
  const customComponentPrefix = Config.library.customComponentPrefix
  const outputPath = path.join(output, `${customComponentPrefix}/api`)
  let apiContent = minifyObject(wrapApis, Config.compile.wrapApis, Config.min)
  let apiInfo = minifyObject(wrapApisInfo, Config.compile.wrapApis, Config.min)
...
function minifyObject(obj = {}, props = {}, min = false) {
  if (!min) { return obj }
  const _obj = {}
