...
useReducer({
  ...
  XmlTagElement(node, store) {
    ...
    if (tagInfo && tagInfo.type === 1) {
      if (tagAst.type === 'wxs' && tagAst.children.length) {
        ...
        const sjsPath = path.join(this.$node.dist, `../${moduleName}`)
        tagAst.props.src = {
          type: 'double',
          value: [`./${moduleName}`],
        }
    ...
    if (tagInfo && tagInfo.props) {
      for (const _prop in tagInfo.props) {
        const propInfo = tagInfo.props[_prop]
        if (!tagAst.props[_prop]) { continue }
        // missing
        if (propInfo.type === 0) {
          delete tagAst.props[_prop]
        } else if (propInfo.type === 1) {
          const _value = tagAst.props[_prop]
          delete tagAst.props[_prop]
  ...
  processOrderProp(node) {
    ...
    if (prop === 'wx:key' && !/{{/.test(props[prop].value[0])) {
      ...
      const newKey = prop.replace(/wx:/, 'a:')
