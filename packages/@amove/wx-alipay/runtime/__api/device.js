...
const apiObj = {
  startBeaconDiscovery: {
    fn(obj = {}) {
      const startBeaconDiscoveryParams = descObj.startBeaconDiscovery.body.params
      const params = utils.defineGetter(obj, startBeaconDiscoveryParams, (obj, prop) => {
        utils.warn(
    ...
    fn(obj = {}) {
      ...
      my.getBluetoothDevices({
        ...
        success: (res) => {
          const arr = res.devices.map((item) => {
            item.advertisData = utils.changeType(item.advertisData)
            return utils.defineGetter(item, getBluetoothDevicesProps, (obj, prop) => {
              utils.warn(
  ...
  scanCode: {
    fn(obj = {}) {
      const scanCodeSuccessRes = descObj.scanCode.body.successRes
      if (obj.scanType) {
      ...
      my.scan({
        ...obj,
        success(res) {
          const _res = utils.defineGetter(res, scanCodeSuccessRes, (obj, prop) => {
            utils.warn(
  ...
  stopGyroscope: {
    fn(obj = {}) {
      const stopGyroscopeParams = descObj.stopGyroscope.body.params
      const params = utils.defineGetter(obj, stopGyroscopeParams, (obj, prop) => {
        utils.warn(
  ...
  onCompassChange: {
    fn(cb) {
      const onCompassChangeReturnValue = descObj.onCompassChange.body.returnValue
  ...
  stopCompass: {
    fn(obj = {}) {
      const stopCompassParams = descObj.stopCompass.body.params
      const params = utils.defineGetter(obj, stopCompassParams, (obj, prop) => {
        utils.warn(
  ...
  stopAccelerometer: {
    fn(obj = {}) {
      const stopAccelerometerParams = descObj.stopAccelerometer.body.params
      const params = utils.defineGetter(obj, stopAccelerometerParams, (obj, prop) => {
        utils.warn(
  ...
  makePhoneCall: {
    fn(obj = {}) {
      const makePhoneCallParams = descObj.makePhoneCall.body.params
      if (obj.phoneNumber) {
        obj.number = obj.phoneNumber
        delete obj.phoneNumber
      }
      const params = utils.defineGetter(obj, makePhoneCallParams, (obj, prop) => {
        utils.warn(
