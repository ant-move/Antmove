...
const method = {
  report(consoleData) {
    ...
    console.log(
      `[${chalk.gray(time)}]`
        + ` ${status} '${chalk.cyan(reportData)}' ${after} ${chalk.magenta(
          cycle,
        )}`,
    )
  },
...
module.exports = {
  report(date, reportData = {}) {
    let finishDate = Number(new Date())
    ...
    if (reportData.type === 'compile') {
      ...
      finishDate = new Date()
      const cycle = `${finishDate - date}ms`
      const time = method.dateFormat(finishDate, 'hh:mm:ss')
      logData = {
        time,
