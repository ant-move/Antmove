...
my.setStorageSync({
    key: "activeComponent",
    data: {
        is: "page/cloud/pages/db-permission/db-permission"
    }
...
_Page({
        ...
        db.collection(collection)
            ...
            .get({
                success: res => {
                    ...
                    const resFirstData = res.data[0] || {}; // 返回的不是要查询用户的记录，是由于没有读权限，视为查询失败

                    if (
                        resFirstData._openid &&
                        resFirstData._openid !== _openid
                    ) {
    ...
    updateOneByOpenId(
        ...
        const db = _my.cloud.database(); // 限制每人仅存一条记录，先查询是否已存在记录
